...
char *do_crypt(char* text){
    ...
    if(!EVP_EncryptUpdate(&ctx, outbuf, &outlen, encrypt_text, strlen(encrypt_text)))
    {
        /* Error */
        return NULL;
    }
...
char *do_decrypt(char* text, int x, unsigned char *try_this_key){
    ...
    int delen, remainingBytes, i, j = 0;
    ...
    if(!EVP_DecryptUpdate(&ctx, debuf, &delen, decrypt_text, x))
    {
        /* Error */
        return NULL;
    }
...
int do_func(char*input_text, int switch_int){
    ...
    if (switch_int == 1){
        ...
        str_to_send = malloc(encrypted_count);
        memcpy(str_to_send, base64_encoded, encrypted_count);
        ...
        while (i < number_of_keys){
            ...
            detext = do_decrypt(base64_decoded, encrypted_count, collected_key);
            if (detext != NULL){
                ...
                printf("Key [%d] did not work\n", i);
            }
            i++;
        }
        if (i == number_of_keys){
            printf("Out of keys to try! Sorry, not this time. Try again later.\n");
        }
        
        str_to_send = malloc(encrypted_count);
        memcpy(str_to_send, base64_decoded, decrypted_count);
